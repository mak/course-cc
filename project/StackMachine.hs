...
--import Data.Sequence(Seq,(|>), (><),fromList,empty,viewl)
--import Data.Foldable (toList)
import Queue hiding (head,tail)

data StackMachine = SM {
    _stack :: [Double ]
   ,_rets :: [ Int] 
   ,_output :: String -- Queue  Char
   ,_input :: [Double] 
   ,_code :: CodeBase
   ,_codePtr :: Int 
   }
...
step OutputChar = \sm -> let n = head $ get stack sm in mod  output (++[(chr $ fromEnum  n)]) sm  --  `snoc` )  sm
step Input = \sm -> let n = head $ get input sm in mod stack (n:) . mod input tail $ sm
